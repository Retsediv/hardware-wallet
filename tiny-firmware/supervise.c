...
static void svhandler_flash_program(uint32_t psize)
{
    ...
    if (psize != FLASH_CR_PROGRAM_X8 && psize != FLASH_CR_PROGRAM_X16 && psize != FLASH_CR_PROGRAM_X32 && psize != FLASH_CR_PROGRAM_X64)
        return;
    FLASH_CR = (FLASH_CR & ~(FLASH_CR_PROGRAM_MASK << FLASH_CR_PROGRAM_SHIFT)) | (psize << FLASH_CR_PROGRAM_SHIFT);
